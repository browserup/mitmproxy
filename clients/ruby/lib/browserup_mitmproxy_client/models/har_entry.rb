...
module BrowserupMitmProxy
  class HarEntry
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BrowserupMitmProxy::HarEntry`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'_trace_id')
        self._trace_id = attributes[:'_trace_id']
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] timings Value to be assigned
    def timings=(timings)
      if timings.nil?
        fail ArgumentError, 'timings cannot be nil'
      end
    ...
    def hash
      [pageref, started_date_time, time, request, response, cache, timings, server_ip_address, _web_socket_messages, _span_id, _parent_id, _trace_id, connection, comment].hash
    end
