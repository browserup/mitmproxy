...
class HarDummyTrafficAddon:
    ...
    def request(self, flow: http.HTTPFlow):
        ...
        flow.metadata["dummy_har_entry"] = create_default_har_entry(
            url=flow.request.url, method=flow.request.method, status_code=status_code
        )
    ...
    def response(self, flow: http.HTTPFlow):
        ...
        for key, value in default_har.items():
            if (
                key in har_entry
                and isinstance(har_entry[key], dict)
        ...
        logging.info(
            "Successfully updated HAR entry with default values and custom overrides"
        )
