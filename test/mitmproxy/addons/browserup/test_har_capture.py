...
class TestHARCapture:
    ...
    def test_trace_header_preservation(self, hc):
        ...
        with taddons.context(hc) as ctx:
            ...
            f.request.headers["traceparent"] = (
                f"00-{existing_trace_id}-{existing_parent_id}-01"
            )
    ...
    def test_trace_linking(self, hc):
        ...
        with taddons.context(hc) as ctx:
            ...
            assert (
                hc.har["log"]["entries"][1]["_parent_id"]
                == hc.har["log"]["pages"][1]["_span_id"]
            )
    ...
    def test_w3c_trace_context_full_scenario(self, hc):
        ...
        with taddons.context(hc) as ctx:
            ...
            f1.request.headers["traceparent"] = (
                f"00-{original_trace_id}-{original_parent_id}-01"
            )
